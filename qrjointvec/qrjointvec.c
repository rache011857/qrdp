...
void rwish(int df, int p, double **Sigma, double **S){
  ...
  chol(R, p, 1.0e-8, pivot, rank, p, d, Sigma, 0, 0, 1.0e-10);
  GetRNGstate();
  for(i = 0; i < p; i++) Z[i][i] = sqrt(rchisq(df - i));
  for(i = 0; i < p; i++) for(j = i+1; j < p; j++) {
     Z[i][j] = rnorm(0.0, 1.0);
     Z[j][i] = 0.0;
  }
  PutRNGstate();
  mmprod(Z, R, Q, p, p, p, 0, 0, 0);
  mInprod(p, p, Q, S, 0);
...
void cov2cor(int p, double **Sigma, double **R){
  double *diag = vect(p);
  int i, j;
  for (i = 0; i < p; i++) {diag[i] = 1.0/sqrt(Sigma[i][i]); R[i][i] = 1.0;}
  for (i = 0; i < p; i++) for (j = i + 1; j < p; j++) {
    R[i][j] = Sigma[i][j] * diag[i] * diag[j];
    R[j][i] = R[i][j];
  }
}
...
void solvePsd(int p, double **S, double **Sinv){
  ...
  chol(R, p, 1.0e-8, pivot, rank, p, d, S, 0, 0, 1.0e-10);
  backsolve(p, R, Rinv);  
  mInprod(p, p, Rinv, Sinv, 1);
...
double detPsd(int p, double **Sigma, int logarithm, int inv){
  ...
  if (!inv) {
    if (!logarithm) {
      for (result = 1.0, i = 0; i < p; i++) result *= R[i][i] * R[i][i];
    } else {
        for (result = 0.0, i = 0; i < p; i++) result += log(R[i][i]);
        result *= 2.0;
    }
  } else { 
      if (!logarithm) {
...
double gauCopDen(int p, double *x, double **R, int logarithm){
  double *z = vect(p), mdSq, *zn = vect(p), **Q = mymatrix(p, p), **Qinv = mymatrix(p, p), *d = vect(p);
  int *pivot = ivect(p), *rank = ivect(1), i, j; 
  for (i = 0; i < p; i++) {z[i] = qnorm(x[i], 0.0, 1.0, 1, 0);}
  chol(Q, p, 1.0e-8, pivot, rank, p, d, R, 0, 0, 1.0e-10);
  backsolve(p, Q, Qinv);
  triprod(Qinv, p, p, z, zn, 1);  
  mdSq = inprod(zn, zn, p) - inprod(z, z, p);
  if (logarithm){
    return(-mdSq/2.0);
  } else{
      return(exp(-mdSq/2.0));
